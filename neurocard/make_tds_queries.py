...
flags.DEFINE_string(
    'tds_light_csv', 'queries/tds-light.csv',
    'Path to the original tds-light queries; used for loading templates.')
...
flags.DEFINE_string(
    'db', 'dbname=tpcds host=127.0.0.1',
    'Configs for connecting to Postgres server (must be launched and running).')
flags.DEFINE_string('output_csv', 'tds-light-range-spark.csv',
...
TDS_LIGHT_OUTER_CARDINALITY = 2128877229383
...
def MakeQueries(spark, cursor, num_queries, tables_in_templates, table_names,
    ...
    for table_name in table_names:
        categorical_cols = datasets.TPC_DS.CATEGORICAL_COLUMNS[
            table_name]
